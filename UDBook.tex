...
\subsection{Durham and Flournoy's  Biased Coin Design}
...
\sqrt{n}\left(W_1(n),\ldots, W_m(n)-\boldsymbol{\pi}^\prime\right)\,
...
eigenvalues in expressing the limiting variance of a Markov chain.
 \citep[see also][get citation for the new edition]{Keme:Snel:1960} \citep(insert citation for Grinstead \& Snell Chapter 11, pg 464).
...
Suppose again that a subject has just been allocated to dose $d_k$, and let $r_k$ denote the mean time is takes the dose allocation process to return to dose $d_k$.  Then $$r_k=\frac{1}{\pi_k}. $$
...
\end{equation*}
is called the \emph{fundamental matrix for regular Markov chains}.  We show below that
...
\subsection{The Continual Reassessment Method }
\begin{enumerate}
